...
| 评估项         | 状态        | 说明                                                         |
| -------------- | ----------- | ------------------------------------------------------------ |
| **设计完整性** | ✅ 可运行   | 核心 Borrow / Repay 主路径已打通；当前重点转为入口收敛、职责边界与一致性校验 |
| **模块化程度** | ✅ 良好     | 模块拆分合理，职责清晰                                       |
| **安全性**     | ⚠️ 需要关注 | 双入口风险、缓存一致性需要重点测试                           |
| **可维护性**   | ⚠️ 待改进   | VaultLendingEngine 过大，需要拆分                            |
| **文档准确性** | ⚠️ 需持续校正 | 核心命名与主流程口径已基本对齐，但仍需持续清理历史分析残留与过时示例 |
...
#### 文档描述（参见 Architecture-Guide.md 中 VaultCore 极简入口与代码示例章节）：
...
根据 `SmartContractStandard.md` 中“3.2 模块与动作常量规范（ModuleKeys / ActionKeys）”以及 `ActionKeys.sol` 的实际实现：
...
`Architecture-Guide.md` 中“双架构命名与常量规范”章节规定：

- 私有变量使用 `_camelCase` 格式
- 需要暴露底层存储值时，优先通过显式 getter 使用 `*Var` / `*AddrVar` 后缀
...
`Architecture-Guide.md` 中关于“统一走 VaultCore -> LendingEngine 账本入口”的职责说明：
...
### 9. VaultRouter 命名与职责口径 ⚠️ **中风险**

#### 当前实现口径

`VaultRouter` 当前不是业务查询聚合器，而是**路由与转发协调层**：

```solidity
function processUserOperation(...) external onlyVaultCore {
  // 将 deposit / withdraw 转发到 CollateralManager
}
```

它保留的能力应按以下口径理解：

- 写路径转发：接收来自 `VaultCore` 的 `processUserOperation` 与各类 `push*`，并转发到 View/缓存模块
- 基础设施读接口：仍保留少量路由器自身状态的只读能力，例如缓存健康检查
- 非职责范围：不再承担面向业务方的聚合查询接口，也不再提供历史测试模式或兼容 harness

#### 当前风险点

- ⚠️ 若文档将其笼统写成“纯 View”或“完全无读接口”，会误导权限设计与审计边界
- ⚠️ 若把历史测试/降级方案写成当前能力，会误导脚本与测试维护者寻找不存在的入口

#### 当前应采用的文档口径

- `VaultRouter` 已完成从旧命名向“Router / Forwarder”语义的收敛
- 生产职责以 `processUserOperation`、`push*` 转发和缓存维护为主
- 只保留当前代码中实际存在的接口与行为描述；历史测试模式、测试辅助函数、降级事件口径一律删除
...
`Architecture-Guide.md` 中 Registry 存储设计章节使用固定槽位：
...
`Architecture-Guide.md` 中 SettlementManager / 清算流程章节描述的清算流程涉及多个模块：
...
- 直接调用 `RewardManagerCore.onLoanEventByOrder` 必须触发错误 `RewardManagerCore__UseRewardManagerEntry`
- 不允许保留任何 no-op 兼容入口或旁路脚本路径

#### 验证重点

- Reward 相关测试必须覆盖直接调用拒绝与标准路径成功
- 全仓脚本/测试不得再直接调用 `RewardManagerCore.onLoanEventByOrder*`
- Reward 状态变化必须与账本落账顺序保持一致

### 13. Gas 估算可能不准确 ⚠️ **低风险**

#### 问题描述

`Architecture-Guide.md` 中 Gas 成本估算表声明：
| 更新类型 | 双架构方案 |
|----------|------------|
| 权限更新 | 21,000 gas |
| 位置更新 | 25,000 gas |
...
> **验证状态**：以下问题已通过代码级分析确认存在

### 16. 借贷流程控制流与 Reward 触发约束

#### 当前规范

- Borrow 放款与订单创建不再通过旧 `VaultCore.borrow(...)` 用户入口表达，统一遵循 Funds-Flow SSOT
- Repay/Settle 入口按产品线区分：legacy / 通用订单走 `SettlementManager.repayAndSettle(...)`；blocks-only 订单走 `BlocksOnlyCoordinator.repayBlocks(...)` 与 `settleOrLiquidateBlocks(...)`
- VaultRouter 仅处理 deposit/withdraw 路由与 View push，不承接借还账本写入
- Reward 只允许在账本落账成功后由标准链路触发

#### 一致性要求

- 写入账本不经 View
- orderId、费用路由与 Reward 触发顺序必须与 Funds-Flow 文档一致
- View 推送失败只能按 best-effort 处理，不能反向改变账本与 Reward 的主路径语义

#### 数据流路径（SSOT）

借款/还款的完整资金链（包含出金、费用分发、orderId 落地与 View 推送的先后关系）以 Funds-Flow 文档为唯一权威：

- [`docs/Usage-Guide/Funds-Flow-Architecture-Guide.md`](Usage-Guide/Funds-Flow-Architecture-Guide.md)
...
- **入口收敛（强约束）**
  - 允许的唯一路径：`LendingEngine (OrderEngine)` → `RewardManager.onLoanEventByOrder(...)` → `RewardManagerCore.onLoanEventByOrder(...)`
  - `RewardManagerCore.onLoanEventByOrder` 必须拒绝任何非 `RewardManager` 的直接调用：
    - revert：`RewardManagerCore__UseRewardManagerEntry`
    - 不再保留 no-op 或兼容审计事件，避免错误调用被误判为成功
- **代码级检查（grep/CI 可执行）**
  - 全仓库不应出现除 `src/Reward/RewardManager.sol` 之外的：
    - `RewardManagerCore(...).onLoanEventByOrder(`（按订单入口）
  - 业务链路（LE）优先调用 `IRewardManagerByOrderWithLender.onLoanEventByOrderWithLender(...)`，并在不支持时回退到 `IRewardManagerByOrder.onLoanEventByOrder(...)`（最佳努力 try/catch 允许失败不回滚）
- **测试/脚本覆盖（至少其一满足）**
  - 单元/集成测试：断言“直接调用 RMCore 会 revert（UseRewardManagerEntry）”，且标准入口可正常触发 Easy 发放/扣罚。
  - E2E：通过 `RewardView` 验证 Easy 相关数据（earned/burned/penaltyLedger 等）随 borrow/repay 的落账路径发生变化。
...
- `borrow`：账本落账成功后 best-effort 通知 `Registry[KEY_RM]`
- `repay`：仅在“足额结清”时 best-effort 通知 `Registry[KEY_RM]`
- 回调优先级：先尝试 `onLoanEventByOrderWithLender(...)`，不支持再回退 `onLoanEventByOrder(...)`
- Reward 触发失败不得阻塞账本主流程

#### 调用模式示意

```solidity
// LendingEngine.sol（示意，省略 outcome 计算与参数细节）
address rm = Registry(_registryAddr).getModuleOrRevert(ModuleKeys.KEY_RM);

// Prefer order-based callback with lender/asset.
bool notified = false;
try IRewardManagerByOrderWithLender(rm).onLoanEventByOrderWithLender(
  borrower,
  lender,
  asset,
  orderId,
...
if (!notified) {
  try IRewardManagerByOrder(rm).onLoanEventByOrder(
    borrower,
    orderId,
    principal,
    maturityBlock,
...
- `test/Vault/VaultLendingEngine.refactor.test.ts`
- `test/Vault/VaultLendingEngine.dual-entry.test.ts`
- 与 Reward 落账顺序相关的端到端测试
...
**LendingEngineCore._pushHealthStatus() 当前实现：**
...
历史上，`VaultLendingEngine._pushUserPositionToView` 曾承担直接仓位推送职责。

#### 代码验证证据

**VaultLendingEngine._pushUserPositionToView() 当前实现：**
...
- ⚠️ 该 legacy shim 本身已不再承担仓位推送，因此“直接调用 View 导致主流程回滚”的风险判断不应继续基于这个函数。
- ⚠️ 当前真正需要关注的是 delta 推送主路径的可观测性、幂等性和失败告警，而不是 compat shim 的调用语义。

#### 改进建议

- 将分析、测试与运维口径统一收敛到 `LendingEngineCore.borrow/repay/forceReduceDebt` 的 delta 推送路径，避免后续文档继续引用 legacy shim 作为权威实现。

#### 当前实现与测试（已对齐 Architecture-Guide）

- 实现情况：`VaultLendingEngine._pushUserPositionToView` 已降级为 legacy no-op compat shim；当前仓位/统计缓存推送的权威路径为 `LendingEngineCore.borrow/repay/forceReduceDebt` 中的 delta 推送与失败告警逻辑，主流程不再依赖该 shim 直接调用 View。
- 覆盖测试：
...
- 实现情况：`VaultCore.sol` 已添加 `viewContractAddrVar()` 公开方法，供各业务/清算模块通过 `IVaultCoreMinimal` 接口解析 VaultRouter 地址使用。
- 代码位置：
...
| 3    | **Reward 触发链路验收** ✅              | 保持 LendingEngine 落账后按标准路径触发 RewardManager | 核心开发 |
...
| 16   | **验证 Reward 模块入口** ✅ | 确保仅保留标准入口且无遗留直接调用                                                                  | QA 团队  |
...
| 风险等级         | 问题数 | 问题列表                                                                                                                   |
| ---------------- | ------ | -------------------------------------------------------------------------------------------------------------------------- |
| 🔴 **P0 阻断性** | 3      | **借贷流程断裂** ✅、**Reward 触发链路验收** ✅、**viewContractAddrVar缺失** 🆕                                            |
...
| 类型             | 问题列表                                                                        |
| ---------------- | ------------------------------------------------------------------------------- |
| **核心逻辑缺陷** | **借贷流程断裂** ✅、**Reward 触发链路验收** ✅、**viewContractAddrVar缺失** 🆕 |
...
3. **持续验证 LendingEngine → RewardManager 标准触发链路**（问题16），确保激励系统不出现入口回退。
...
| 版本 | 更新内容                                                                                             |
| ---- | ---------------------------------------------------------------------------------------------------- |
| 2.2  | **二次验证**：通过代码级分析确认关键问题，并补充 `viewContractAddrVar` 等实现一致性检查 |
| 2.1  | **紧急审计**：补充借贷流程、奖励集成与阻断性缺陷的排查结论                                 |
| 2.0  | 基于最新架构分析报告全面更新，补充问题清单与优先级                                         |
| v1.0 | 初始版本                                                                                             |
