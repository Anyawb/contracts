...
- **清算编排入口唯一（`KEY_LIQUIDATION_MANAGER`）**
...
- **旧路径（Legacy / 旧模块族 / 旧清算族）**  
  指历史遗留的“第二入口”“经 View/业务层转发写入”“旧 key 绑定”“旧清算模块族（如 LCM/LiquidationViewLibrary 等）”等，与目标架构的职责边界冲突或重复。

### 术语映射（把“方案A/方案B”翻译成文档一致的说法）

| 你任务里的叫法 | 建议统一叫法 | 核心特征（用于验收） |
|---|---|---|
| 方案A（目标） | **直达账本清算架构（Direct-to-Ledger）** | `KEY_LIQUIDATION_MANAGER` 编排 → `KEY_CM` 扣押 → `KEY_LE` 减债 → `LiquidatorView.push*` 单点 DataPush |
...
**清算由 `LiquidationManager`（`KEY_LIQUIDATION_MANAGER`）作为唯一编排入口发起，写入操作只直达账本模块 `CollateralManager` 与 `LendingEngine`，写入成功后仅由 `LiquidatorView.pushLiquidationUpdate/Batch` 进行单点 DataPush；任何 View 不承载写入转发，任何业务层不保留第二清算入口。**
...
- **编排入口**：`Registry.KEY_LIQUIDATION_MANAGER` → `LiquidationManager`
...
- Registry 中 **`KEY_LIQUIDATION_MANAGER` 只指向一个合约地址**（目标为 `LiquidationManager`）。
...
一旦决定 `LiquidationManager` 为唯一清算编排入口：

- `Registry.KEY_LIQUIDATION_MANAGER` 只绑定 `LiquidationManager`
