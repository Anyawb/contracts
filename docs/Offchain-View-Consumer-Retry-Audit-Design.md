...
### 2.4 blocks-only runtime 收敛补充

- blocks-only 订单状态不属于本文件描述的通用 View push 主路径；链下若消费 blocks-only 订单，应优先把 `BlocksOnlyCoordinator` 事件与 `BlocksOnlyView` runtime 组合成读模型，而不是等待通用 cache push。
- 对 blocks-only repay 后的收敛，链下应优先观察 `remainingDebt` 与 `canCloseTrade`，不要把 `maturityBlock` 当成 debt-free close 的唯一推进条件。
- 在 `termBlocks = 1` 下，还款后的下一次 runtime 读取可能同时观察到 `isMatured = true` 与 `canCloseTrade = true`；链下状态机必须允许这种并存。
...
#### CacheEvents (统一缓存失败观测)

| 合约                                          | Event                          | Canonical Signature                                                                                 | Indexed（topics）                                             | 非 indexed（data）                                                                                                                | 链下建议                                                                                 |
| :-------------------------------------------- | :----------------------------- | :-------------------------------------------------------------------------------------------------- | :------------------------------------------------------------ | :-------------------------------------------------------------------------------------------------------------------------------- | :--------------------------------------------------------------------------------------- |
| `CacheEvents`<br/>`src/Vault/CacheEvents.sol` | `CacheUpdateFailed`            | `CacheUpdateFailed(address,address,address,uint256,uint256,bytes)`                                  | `user: address`<br/>`asset: address`                          | `viewAddr: address`<br/>`collateral: uint256`<br/>`debt: uint256`<br/>`reason: bytes`                                             | 失败入队来源（旧版或无上下文接口）；由具体模块触发，需通过 `contract_address` 区分来源。 |
| `CacheEvents`                                 | `CacheUpdateFailedWithContext` | `CacheUpdateFailedWithContext(address,address,bytes32,address,uint256,uint256,bytes,uint64,uint64)` | `user: address`<br/>`asset: address`<br/>`requestId: bytes32` | `viewAddr: address`<br/>`collateral: uint256`<br/>`debt: uint256`<br/>`reason: bytes`<br/>`seq: uint64`<br/>`nextVersion: uint64` | **带回放上下文的失败事件**：包含幂等键 (`requestId`)，是主推的重试线索。                 |

#### HealthEvents (统一健康度推送观测)

| 合约                                            | Event              | Canonical Signature                                       | Indexed（topics）                         | 非 indexed（data）                                                      | 链下建议                                                |
| :---------------------------------------------- | :----------------- | :-------------------------------------------------------- | :---------------------------------------- | :---------------------------------------------------------------------- | :------------------------------------------------------ |
| `HealthEvents`<br/>`src/Vault/HealthEvents.sol` | `HealthPushFailed` | `HealthPushFailed(address,address,uint256,uint256,bytes)` | `user: address`<br/>`healthView: address` | `totalCollateral: uint256`<br/>`totalDebt: uint256`<br/>`reason: bytes` | 健康因子推送失败监控/重试来源；主要用于告警与运维排查。 |
...
- upsert `position_cache_mirror(user, asset)`：
  ...
  - 否则更新 collateral/debt/version/last*chain*\* 等字段
...
### 7.2 重试执行（建议默认路径：B+ 单入口编排器）

默认建议调用链上统一编排器的 Admin/Retry 入口（而不是尝试重放 Delta）：

- 对于用户仓位：调用 `PositionPushManager.retryUserPositionUpdate(user, asset)` (如有对应的 PushManager 编排器)
- 对于协议级聚合：调用 `StatisticsPushManager.retryUserStats(user)` / `retryGuarantee(user, asset)`

理由：

- **严格 B+ (Snapshot) 架构** 强调重新计算全量状态后再写入。此入口会“重读账本（CM/LE）/ 快照 → 写缓存 → 发出包含 `version` 的最终事件”。
- 链下不用自己计算 delta/版本，减少复杂度与覆盖风险，防止并发乱序下越修越错。

### 7.3 重试前置检查（必须）

- **租约锁**：对 `(user, asset, view)` 抢锁（Redis/DB row lock），避免并发重试
- **冷却时间**：`now() >= next_available_at` 才允许执行
- **模块缓存/升级窗口**：如遇 `ModuleCacheExpired` 或依赖缺失，应由运维触发 `CacheMaintenanceManager.batchRefresh(targets)` 修复链上模块引用后再重试。
...
- 执行重写（如 `PushManager.retry*`）和刷新缓存（如 `CacheMaintenanceManager.batchRefresh`）的签名账户只由后端/运维持有，必须区分角色，硬件钱包/多签更佳。
