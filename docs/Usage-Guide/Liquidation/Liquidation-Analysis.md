...
- 架构指南要求的模块键：入口 `KEY_SETTLEMENT_MANAGER`（SSOT）、执行器 `KEY_LIQUIDATION_MANAGER`、账本 `KEY_CM`/`KEY_LE`、事件 `KEY_LIQUIDATION_VIEW`、风控只读 `KEY_LIQUIDATION_RISK_MANAGER → LiquidationRiskManager`（只读聚合）；其他清算子模块不再注册。