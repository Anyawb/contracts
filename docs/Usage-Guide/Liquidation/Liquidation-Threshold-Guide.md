...
清算阈值是RWA借贷平台的核心风险控制参数，用于判断用户是否可以被清算。当用户的健康因子低于清算阈值时，系统允许清算人执行清算操作，确保平台资产安全。本文已对齐 `Architecture-Guide copy.md`：阈值存储与校验在风控/账本侧（如 `LiquidationRiskManager`/`LendingEngine`），只读层仅查询健康因子/阈值；清算写入直达账本，事件/DataPush 由 `LiquidatorView.pushLiquidationUpdate/Batch` 单点触发。
