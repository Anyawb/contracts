...
- **角色（role）不是入口收敛的替代品**：如果任意 EOA 拿到 `ACTION_LIQUIDATE`，就能绕过“清算执行器模块”直接写账本，会造成：
  ...
  - 审计面扩大（任何持权地址都是潜在写入口）
- **最佳实践（与架构指南一致）**：
  - 写入口必须收敛到 SSOT 模块（例如架构指南中的 `SettlementManager` 统一结算/清算写入口；或清算执行器 `LiquidationManager`）。
...
- `src/Vault/modules/LenderPoolVault.sol`（资金池托管与资金流转：出借准备金/撮合放款/回款资金承接）
- `src/Vault/modules/GuaranteeFundManager.sol` / `EarlyRepaymentGuaranteeManager.sol`（保证金相关）
- `src/Vault/liquidation/modules/SettlementManager.sol`（统一结算/清算写入口 SSOT：还款结算/到期处置/被动清算）
...
- `src/core/LendingEngine.sol`（订单/撮合/放款落地：资金链与账本写入的核心编排之一，需纳入资金链审计范围）
...
- `src/access/AccessControlManager.sol`（权限 SSOT：ACTION_* 角色控制资金链/清算入口）
...
- **P2（View/监控）定向测试**：
  - `pnpm -s test test/Vault/view test/Vault/view/modules test/core/CoinGeckoPriceUpdater.test.ts`
