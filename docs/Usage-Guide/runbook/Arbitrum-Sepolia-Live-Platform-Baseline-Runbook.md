...
4. 如必须单跑某个写链脚本，也要在同一轮终端环境里补跑 [sweep-fresh-borrowers.ts](../../../scripts/tests/live-test/sweep-fresh-borrowers.ts)，否则本轮测试不算完成。
...
```bash
set -a && source .env && set +a && pnpm -s run test:live:platform-baseline:arbitrum-sepolia
```

### 3. 上线前跑统一 release gate
...
| [live-release-gates-arbitrum-sepolia.ts](../../../scripts/tests/live-test/live-release-gates-arbitrum-sepolia.ts) | `本轮通过` | 统一发布入口 | 顺序执行四个 gate 并产生日志与汇总 |
| [live-platform-baseline-arbitrum-sepolia.ts](../../../scripts/tests/live-test/live-platform-baseline-arbitrum-sepolia.ts) | `本轮通过` | 平台整体资金链基线 | normal borrow/repay + Easy 闭环 + guarantee default -> liquidation |
...
| [live-blocks-only-liquidation-arbitrum-sepolia.ts](../../../scripts/tests/live-test/live-blocks-only-liquidation-arbitrum-sepolia.ts) | `本轮通过` | blocks-only 收尾 | repayBlocks、settle、liquidate、runtime/DataPush |
...
2. `live-platform-baseline-arbitrum-sepolia.ts`
...
1. [live-platform-baseline-arbitrum-sepolia.ts](../../../scripts/tests/live-test/live-platform-baseline-arbitrum-sepolia.ts)：覆盖 normal repay 后 `EASY_MINTED`、`EASY_SPENT`、`EASY_RECYCLED_SPLIT`，以及 guarantee default -> liquidation -> reward penalty/burn 正向链路。
2. [live-view-reward-loanflow-boundary-arbitrum-sepolia.ts](../../../scripts/tests/live-test/live-view-reward-loanflow-boundary-arbitrum-sepolia.ts)：覆盖 RewardView 读面、LoanFlowView 边界与 borrow/repay 后的 reward 读面推进。
3. [live-reward-config-governance-arbitrum-sepolia.ts](../../../scripts/tests/live-test/live-reward-config-governance-arbitrum-sepolia.ts)：覆盖 reward 治理 caller gate 与幂等 no-op 写回。
4. [live-easy-staking-arbitrum-sepolia.ts](../../../scripts/tests/live-test/live-easy-staking-arbitrum-sepolia.ts)：覆盖 repay 后拿到 EASY，再走 stake / unstake 闭环。
...
2. 检查 relayer 是否缺 `LIQUIDATE`、`ACTION_VIEW_PUSH`、`ACTION_ADMIN` 等角色。
...
1. relayer 具备 `VIEW_PRICE_DATA`、`LIQUIDATE`、`DEPOSIT`、`VIEW_SYSTEM_DATA`、`ACTION_VIEW_PUSH`
2. SettlementManager 具备 `REPAY`、`VIEW_SYSTEM_DATA`
