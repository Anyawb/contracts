...
async function main() {
  ...
  try {
    ...
    if (!deployed.AICreditsVault) {
      try { deployed.AICreditsVault = await deployProxy('src/core/AICreditsVault.sol:AICreditsVault', [deployed.Registry]); save(deployed); } catch (error) { console.log('⚠️ AICreditsVault deployment failed:', error); }
    }
    ...
    if (!deployed.FeatureRegistry) {
      try { deployed.FeatureRegistry = await deployProxy('src/Reward/FeatureRegistry.sol:FeatureRegistry', [deployed.Registry]); save(deployed); } catch (error) { console.log('⚠️ FeatureRegistry deployment failed:', error); }
    }
    if (!deployed.GovernanceGate) {
      try { deployed.GovernanceGate = await deployProxy('src/Governance/GovernanceGate.sol:GovernanceGate', [deployed.Registry]); save(deployed); } catch (error) { console.log('⚠️ GovernanceGate deployment failed:', error); }
    }
    ...
    try {
      if (deployed.AccessControlManager) {
        ...
        if (deployed.RewardConfig) {
          ...
          if (!has) {
            await (await acm.grantRole(SET_PARAMETER, deployed.RewardConfig)).wait();
