/**
 * BNB Testnet 部署脚本（符合 contracts/docs/Architecture-Guide.md）
 * BNB Testnet Deployment Script
 *
 * 该脚本用于将智能合约系统部署到 BNB Testnet
 * This script deploys the smart contract system to BNB Testnet
 * - 部署 Registry（单一入口 / 单一 Proxy，Scheme A）
 * - 部署并注册核心业务与视图模块
 * - 写入 scripts/deployments/bnb-testnet/core.json 与 frontend-config/contracts-bnb-testnet.ts
 */
...
const BNB_TESTNET_CONFIG = {
  ...
  frontendSlug: "bnb-testnet",
  verifyApiKeyEnv: "BSCSCAN_API_KEY",
  backupPrefix: "bnb-testnet",
...
const MANIFEST_FILE = path.join(DEPLOY_DIR, "bnb-testnet", "manifest.json");
const BASELINE_FILE = path.join(DEPLOY_DIR, "bnb-testnet", "baseline.json");
const BASELINE_HISTORY_DIR = path.join(DEPLOY_DIR, "bnb-testnet", "history");
const MOCK_SUITE_FILE = path.join(DEPLOY_DIR, "bnb-testnet", "mock-suite.json");
...
const FRONTEND_FILE = path.join(
  FRONTEND_DIR,
  `contracts-${BNB_TESTNET_CONFIG.frontendSlug}.ts`,
);
const CANONICAL_FRONTEND_FILE = path.join(
  FRONTEND_DIR,
  "networks",
  `${BNB_TESTNET_CONFIG.frontendSlug}.ts`,
);
const FRONTEND_RELEASE_FILE = path.join(
  FRONTEND_DIR,
  "networks",
  `${BNB_TESTNET_CONFIG.frontendSlug}.release.json`,
);
...
function resolveConfiguredSettlementAsset() {
  return resolveSettlementAssetConfig(
    BNB_TESTNET_CONFIG.name,
    BNB_TESTNET_CONFIG.chainId,
  );
}

function shouldStrictlyFailAssetConfig() {
  return process.env.DEPLOY_STRICT_ASSET_CONFIG !== "0";
}

function shouldAssertPriceUpdaterAfterDeploy() {
  return process.env.DEPLOY_ASSERT_PRICE_UPDATER !== "0";
}
...
function handleConfiguredAssetsFailure(stage: string, error: unknown): never | void {
  if (shouldStrictlyFailAssetConfig()) {
    throw new Error(
      `Configured assets failed during ${stage} for ${BNB_TESTNET_CONFIG.name}: ${formatDeployError(error)}`,
    );
  }
...
function resolveDeployFile() {
  ...
  if (!explicit) return path.join(DEPLOY_DIR, "bnb-testnet", "core.json");
...
async function assertPriceUpdaterReadyForConfiguredAssets(deployed: DeployMap) {
  ...
  if (!deployed.PriceUpdater || !deployed.AccessControlManager) {
    throw new Error(
      `PriceUpdater readiness assert requires deployed PriceUpdater and AccessControlManager on ${BNB_TESTNET_CONFIG.name}`,
    );
  }
  ...
  if (!assets.length) {
    throw new Error(
      `PriceUpdater readiness assert found no configured assets for ${BNB_TESTNET_CONFIG.name}`,
    );
  }
...
function resolveRuntimeOperatorAddresses(deployed: DeployMap) {
  const relayerAddress = process.env.RELAYER_PRIVATE_KEY?.trim()
    ? new ethers.Wallet(process.env.RELAYER_PRIVATE_KEY.trim()).address
    : process.env.PRIVATE_KEY?.trim()
      ? new ethers.Wallet(process.env.PRIVATE_KEY.trim()).address
      : process.env.RELAYER_ADDRESS?.trim() || deployed.GovernanceGuardian;
  const updaterAddress = process.env.UPDATER_PRIVATE_KEY?.trim()
    ? new ethers.Wallet(process.env.UPDATER_PRIVATE_KEY.trim()).address
    : process.env.PRIVATE_KEY?.trim()
      ? new ethers.Wallet(process.env.PRIVATE_KEY.trim()).address
      : process.env.UPDATER_ADDRESS?.trim() || relayerAddress;
  const viewerAddress = process.env.VIEWER_PRIVATE_KEY?.trim()
    ? new ethers.Wallet(process.env.VIEWER_PRIVATE_KEY.trim()).address
    : process.env.VIEWER_ADDRESS?.trim() || relayerAddress;
...
async function ensureRuntimeOperatorRoles(deployed: DeployMap) {
  if (!deployed.AccessControlManager) return;
  const acm = await ethers.getContractAt(
    "AccessControlManager",
    deployed.AccessControlManager,
  );
  const { relayerAddress, updaterAddress, viewerAddress } = resolveRuntimeOperatorAddresses(deployed);

  await ensureAcmRoles(acm, "Runtime operator roles", [
    { roleName: "VIEW_PRICE_DATA", account: relayerAddress, label: "Relayer" },
    { roleName: "VIEW_USER_DATA", account: relayerAddress, label: "Relayer" },
    { roleName: "VIEW_RISK_DATA", account: relayerAddress, label: "Relayer" },
    { roleName: "VIEW_SYSTEM_DATA", account: relayerAddress, label: "Relayer" },
    { roleName: "LIQUIDATE", account: relayerAddress, label: "Relayer" },
    { roleName: "DEPOSIT", account: relayerAddress, label: "Relayer" },
    { roleName: "ACTION_VIEW_PUSH", account: relayerAddress, label: "Relayer" },
    { roleName: "UPDATE_PRICE", account: relayerAddress, label: "Relayer" },
    { roleName: "ACTION_ADMIN", account: relayerAddress, label: "Relayer" },
    { roleName: "ACTION_SET_PARAMETER", account: relayerAddress, label: "Relayer" },
    { roleName: "ACTION_REWARD_CONFIG_EMERGENCY", account: relayerAddress, label: "Relayer" },
    { roleName: "UPDATE_PRICE", account: updaterAddress, label: "Updater" },
    { roleName: "VIEW_USER_DATA", account: viewerAddress, label: "Viewer" },
    { roleName: "VIEW_RISK_DATA", account: viewerAddress, label: "Viewer" },
    { roleName: "VIEW_SYSTEM_DATA", account: viewerAddress, label: "Viewer" },
  ]);
}
...
async function validateDeployerNativeBalance(
  ...
  console.log(
    `账户余额 Balance: ${ethers.formatEther(balance)} ${BNB_TESTNET_CONFIG.nativeSymbol}`,
  );

  if (balance < minBalanceWei) {
    throw new Error(
      `部署账户余额不足，至少需要 ${ethers.formatEther(minBalanceWei)} ${BNB_TESTNET_CONFIG.nativeSymbol} 支付 Gas，当前仅有 ${ethers.formatEther(balance)} ${BNB_TESTNET_CONFIG.nativeSymbol}`,
    );
  }
}
...
async function checkEnvironment(): Promise<void> {
  console.log(`🔍 检查 ${BNB_TESTNET_CONFIG.displayLabel} 环境配置...`);
  console.log(`🔍 Checking ${BNB_TESTNET_CONFIG.displayLabel} environment...`);
  ...
  if (!process.env[BNB_TESTNET_CONFIG.verifyApiKeyEnv]) {
    console.log(`⚠️ 建议配置环境变量: ${BNB_TESTNET_CONFIG.verifyApiKeyEnv}`);
  }

  try {
    if (
      !isLocalFork &&
      runtimeNet.chainId !== BigInt(BNB_TESTNET_CONFIG.chainId)
    ) {
      throw new Error(
        `网络配置错误，期望 Chain ID: ${BNB_TESTNET_CONFIG.chainId}，实际为 ${runtimeNet.chainId}`,
      );
    }
    console.log(
    ...
    throw new Error(`${BNB_TESTNET_CONFIG.displayLabel} 网络连接失败`);
  }
...
async function backupWalletAssets(): Promise<void> {
  ...
  const backupFile = path.join(
    backupDir,
    `${BNB_TESTNET_CONFIG.backupPrefix}-backup-${backupBlock}.json`,
  );

  // 保存备份信息 Save backup information
  const backupData = {
    blockNumber: backupBlock,
    network: BNB_TESTNET_CONFIG.name,
...
async function main() {
  ...
  await configureDynamicEip1559Fees({
    ...
    label: `deploy-${BNB_TESTNET_CONFIG.name}`,
  });
  ...
  try {
    ...
    try {
      await ensureRuntimeOperatorRoles(deployed);
    } catch (error) {
      console.log("⚠️ Grant runtime operator roles skipped/failed:", error);
    }
    ...
    // 4) 生成 baseline、manifest 与前端 release 产物
    const generatedAt = new Date().toISOString();
    const releaseId = process.env.RELEASE_SYNC_RELEASE_ID?.trim()
      || `${BNB_TESTNET_CONFIG.name}-${generatedAt.replace(/[-:TZ.]/g, "")}`;
    const sourceFiles = {
    ...
    await writeDeployBaseline({
      ...
      provider: new JsonRpcProvider(BNB_TESTNET_CONFIG.rpcUrl),
    ...
    writeFrontendArtifacts({
      ...
      core: deployed,
      releaseArtifact,
      displayLabel: BNB_TESTNET_CONFIG.displayLabel,
      network: BNB_TESTNET_CONFIG.name,
      chainId: BNB_TESTNET_CONFIG.chainId,
      rpcUrl: BNB_TESTNET_CONFIG.rpcUrl,
      explorer: BNB_TESTNET_CONFIG.explorer,
    });
    ...
    console.log(`📝 Baseline archived: ${archivedBaselineFile}`);
    ...
    // 5) 输出摘要
    console.log(`\n==== Deployment Addresses (${BNB_TESTNET_CONFIG.name}) ====`);
