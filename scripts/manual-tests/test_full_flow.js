...
async function main() {
  ...
  const RewardManager = await hre.ethers.getContractFactory("MockRewardManager");
  ...
  const KEY_REWARD_MANAGER_V1 = hre.ethers.keccak256(hre.ethers.toUtf8Bytes("REWARD_MANAGER_V1"));
  ...
  await registry.setModule(KEY_REWARD_MANAGER_V1, await rewardManager.getAddress());
  
  await cm.depositCollateral(user.address, debtAsset, 200);
  
  console.log("All modules registered. Testing borrow with trace...");
  ...
  try {
    ...
    if (e.error) {
      console.log("Error object:", JSON.stringify(e.error, null, 2));
    }
    // Try to decode the error
    if (e.data && e.data !== "0x") {
      try {
        const iface = new hre.ethers.Interface(["error VaultLendingEngine__OnlyVaultCore()", "error ZeroAddress()", "error AmountIsZero()"]);
