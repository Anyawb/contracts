...
async function main() {
  ...
  await priceOracle.connect(vaultCore).setPrice(await settlementToken.getAddress(), hre.ethers.parseEther("1"), nowBlock, 18);
  const debtAsset = hre.ethers.Wallet.createRandom().address;
  await priceOracle.connect(vaultCore).setPrice(debtAsset, hre.ethers.parseEther("1"), nowBlock, 18);
  ...
  try {
    ...
    const [price, blockNumber, decimals] = await IPriceOracle.getPrice(debtAsset);
    console.log("IPriceOracle.getPrice result:", price.toString(), blockNumber.toString(), decimals.toString());
  } catch (e) {
  ...
  try {
    ...
    const [price, blockNumber, decimals] = await IPriceOracleAdapter.getPrice(debtAsset);
    console.log("IPriceOracleAdapter.getPrice result:", price.toString(), blockNumber.toString(), decimals.toString());
  } catch (e) {
