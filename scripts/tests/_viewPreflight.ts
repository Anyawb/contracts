...
export async function runViewPreflight(params: {
      ...
      if (!ok) {
        try {
          ...
          if (isRoleAlreadyGranted(e)) {
            ...
            throw new Error(`preflight: missing ACTION_VIEW_PUSH for ${pusher} and failed to grantRole (are you ACM owner?): ${e?.message ?? String(e)}`);
          }
        }
      }
        ...
        if (!hasRisk) {
          try {
            ...
            if (isRoleAlreadyGranted(e)) {
              ...
              throw new Error(`preflight: missing VIEW_RISK_DATA for ${pusher} and failed to grantRole (are you ACM owner?): ${e?.message ?? String(e)}`);
            }
          }
        }
      }
  ...
  const [vp, vBlock, vValid] = (await vov.connect(adminSigner).getAssetPrice(assetForPriceCheck)) as [bigint, bigint, boolean];
  ...
  if (print) {
    ...
    console.log(`  [check] price fallback asset=${assetForPriceCheck} price=${vp.toString()} block=${vBlock.toString()} valid=${String(vValid)}`);
    if (!vValid) {
      console.log("  [note] fallback price is invalid (0/0) — this is expected before PriceOracle is seeded via updatePrice().");
    }
