...
export function requireFeeRouterGate(snapshot: Awaited<ReturnType<typeof observeExtendedViews>>, stage: string) {
  ...
  if (!snapshot.feeRouterSync.isValid || snapshot.feeRouterSync.needsSync) {
    ...
    if (process.env.LIVE_STRICT_FEE_ROUTER_GATE === "1") {
      throw new Error(message);
    }
    console.log(`  [Notice] ${message}; skipping strict publish-ready assertions (set LIVE_STRICT_FEE_ROUTER_GATE=1 to enforce)`);
    return false;
  }
...
function parseReceiptLogsForInterface(receipt: any, contractAddr: string, iface: ethers.Interface) {
  return (receipt?.logs ?? [])
...
function resolveContractAddress(contractOrAddress: any, explicitAddress?: string) {
  const value = explicitAddress ?? contractOrAddress?.target ?? contractOrAddress?.address;
  if (!value) {
    throw new Error("missing contract address for receipt attribution");
  }
  return String(value);
}
...
export function assertFeeDistributionReceiptAttribution(args: {
  ...
  const resolvedFeeRouterAddr = resolveContractAddress(feeRouter, feeRouterAddr);
  const resolvedFeeRouterViewAddr = resolveContractAddress(feeRouterView, feeRouterViewAddr);

  const feeRouterLogs = parseReceiptLogsForInterface(receipt, resolvedFeeRouterAddr, feeRouter.interface);
  ...
  const feeRouterViewLogs = parseReceiptLogsForInterface(receipt, resolvedFeeRouterViewAddr, feeRouterView.interface);
...
export function logFeeRouterViewPushFailures(stage: string, receipt: any, feeRouterInterface: ethers.Interface) {
  const event = (() => {
    try {
