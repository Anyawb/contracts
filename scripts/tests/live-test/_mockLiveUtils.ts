...
// 兼容旧字段 defaultPriceUsd8，避免老 pack 文件直接失效。
export function getAssetBootstrapPriceUsd8(asset: MockAssetPackAsset) {
...
function buildSyntheticAsset(params: {
  ...
  fallbackBootstrapPriceUsd8: string;
}): MockAssetPackAsset {
  ...
  return {
    ...
    maxPriceAge: 3600,
    active: true,
    bootstrapPriceUsd8: envStr(params.bootstrapPriceEnv) ?? params.fallbackBootstrapPriceUsd8,
...
export function loadMockAssetPack(): SelectedMockAssets {
  ...
  if (!settlementAsset && settlementByEnv) {
    settlementAsset = buildSyntheticAsset({
      ...
      bootstrapPriceEnv: "SETTLEMENT_PRICE_UNITS_8",
      fallbackSymbol: "SETTLEMENT",
      fallbackDecimals: 18,
      fallbackBootstrapPriceUsd8: "1",
    });
  }
  if (!settlementAsset) {
  ...
  if (borrowAddress) {
    ...
    if (!borrowAsset) {
      borrowAsset = buildSyntheticAsset({
        ...
        bootstrapPriceEnv: "BORROW_PRICE_UNITS_8",
        fallbackSymbol: "BORROW",
        fallbackDecimals: settlementAsset.decimals,
        fallbackBootstrapPriceUsd8: getAssetBootstrapPriceUsd8(settlementAsset),
      });
    }
...
export async function withNonceManagedSigner<T extends any>(signer: T): Promise<T> {
  ...
  const sendTransaction = async (tx: any) => {
    const withPendingNonce = {
      ...tx,
      nonce: tx?.nonce ?? await loadSuggestedNonce(),
    };
    try {
      return await baseSendTransaction(withPendingNonce);
    } catch (error: any) {
      const message = String(error?.message ?? error?.shortMessage ?? "").toLowerCase();
      if (!message.includes("nonce too low")) {
        throw error;
      }
      const reportedStateNonce = Number(String(error?.message ?? error?.shortMessage ?? "").match(/state:\s*(\d+)/i)?.[1] ?? "NaN");
      const nextNonce = Number.isFinite(reportedStateNonce)
        ? Math.max(await loadSuggestedNonce(), reportedStateNonce)
        : await loadSuggestedNonce();
      return await baseSendTransaction({
        ...tx,
        nonce: nextNonce,
      });
    }
...
export function getReadCaller(label: string, addressEnv: string, fallbackSigner: any) {
  const address = envStr(addressEnv);
  if (address) {
    return {
