...
// fork 版和 localhost 版目标一致：
// 用一条临时 fork 链复现“缺最终价格时 backend-required warmup 必须失败”的行为。
...
// fork 环境需要显式校验 chainId，避免误连到错误节点。
...
// 启动独立 fork node，并将日志持续写入文件，便于排查上游 RPC 抖动或 fork 崩溃。
...
// 每一步都落盘日志，确保能区分是 fork 基础设施问题还是预期的业务阻断。
...
async function main() {
  ...
  try {
    ...
    // 在 fork 上仍复用 localhost 部署链路，目的是稳定复现实验，而不是依赖线上现成状态。
    const baseEnv = {
      ...process.env,
    ...
    const prepare = runStep(
      ...
      ["-s", "exec", "hardhat", "run", "scripts/tests/live-test/prepare-backend-required-block-local.ts", "--network", "localhost"],
    ...
    const warmup = runStep(
      ...
      ["-s", "exec", "hardhat", "run", "scripts/tests/live-test/live-warmup-arbitrum-sepolia.ts", "--network", "localhost"],
      {
        ...baseEnv,
    ...
    // 失败信息必须命中目标文案，才能证明是 price mode 触发的阻断。
