...
async function main() {
  ...
  if (!batchUpdatePush) {
    throw new Error("missing LIQUIDATION_BATCH_UPDATE DataPushed on batchLiquidate receipt");
  }
  ...
  ) as [string[], string[], string[], bigint[], bigint[], string, bigint[], bigint];
  ...
  if (payoutPushes.length < seeded.length) {
    throw new Error(`expected >= ${seeded.length} LIQUIDATION_PAYOUT pushes after batchLiquidate, got ${payoutPushes.length}`);
  }
  ...
  console.log("\n✅ live-batch-liquidation-pressure-arbitrum-sepolia PASSED\n");
}

void runWithNetworkRetry("live-batch-liquidation-pressure-arbitrum-sepolia", main);