...
function getLastDataPush(receipt: any, iface: ethers.Interface, dataTypeHash: string) {
  return (receipt.logs ?? [])
...
async function main() {
  ...
  try {
    ...
    assertFeeDistributionReceiptAttribution({
      ...
      feeRouterView,
      feeRouterViewAddr: ctx.feeRouterViewAddr,
      ...
      expectedPushBlock: afterViews.feeRouterSync?.lastSyncBlock,
    });
    ...
    console.log("\n✅ live-fee-dynamic-gate-arbitrum-sepolia PASSED\n");
  } finally {
    if (wroteTempConfig) {
...
main().catch((error) => {
  console.error("\n❌ live-fee-dynamic-gate-arbitrum-sepolia FAILED\n");
  console.error(error);
  process.exit(1);
});