...
import { getAssetBootstrapPriceUsd8, key } from "./_mockLiveUtils";
...
function formatDecodedArg(value: unknown) {
...
function decodeFailureReason(reason: string, errorInterface?: any) {
  if (!reason || reason === "0x") {
...
async function refreshPrice(ctx: FundsFlowLiveContext, asset: string, label: string) {
  ...
  if (!price || price === 0n || !blockNumber || blockNumber === 0n) {
    ...
    price = BigInt(getAssetBootstrapPriceUsd8(matchedAsset));
  }
...
async function main() {
  ...
  const allDataPushLogs = (liquidationReceipt.logs ?? []).flatMap((log: any) => {
  ...
  const liquidatorLogs = allDataPushLogs.filter((entry) => entry.address === liquidationViewAddr.toLowerCase());
  const settlementLogs = allDataPushLogs.filter((entry) => entry.address === ctx.settlementManagerAddr.toLowerCase());
  ...
  const liquidationUpdateCacheFailures = liquidationManagerCacheFailures.filter(
    (reason) => !reason.includes("pushLiquidationPayout failed")
  ...
  const settlementDataPushTypes = allDataPushLogs
    .filter((entry) => entry.address === ctx.settlementManagerAddr.toLowerCase())
    .map((entry) => dataPushTypeNames.get(String(entry.parsed.args[0]).toLowerCase()) ?? String(entry.parsed.args[0]));
  const liquidatorPushTypes = liquidatorLogs.map(
    (entry) => dataPushTypeNames.get(String(entry.parsed.args[0]).toLowerCase()) ?? String(entry.parsed.args[0]),
  );
  ...
  if (!selectedUpdatePush) {
    if (fallbackActivation) {
      throw new Error(
        `expected LIQUIDATION_UPDATE DataPushed from SettlementManager fallback; fallbackReason=${fallbackReason}; settlementDataPushes=[${settlementDataPushTypes.join(",") || "none"}]`,
      );
    }
    if (liquidationUpdateCacheFailures.length === 0) {
      throw new Error(
        `expected LIQUIDATION_UPDATE observability on liquidation; fallbackActivated=${String(Boolean(fallbackActivation))}; fallbackReason=${fallbackReason}; liquidatorDataPushes=[${liquidatorPushTypes.join(",") || "none"}]; settlementDataPushes=[${settlementDataPushTypes.join(",") || "none"}]; liquidatorViewCacheFailures=[${liquidationManagerCacheFailures.join(" | ") || "none"}]`,
      );
    }
    console.log(
      `  [Notice] liquidation view update push failed after successful ledger write: reasons=[${liquidationUpdateCacheFailures.join(" | ")}]`,
    );
  } else {
  ...
  if (!selectedPayoutPush) {
    if (fallbackActivation) {
      throw new Error(
        `expected LIQUIDATION_PAYOUT DataPushed from SettlementManager fallback; fallbackReason=${fallbackReason}; settlementDataPushes=[${settlementDataPushTypes.join(",") || "none"}]`,
      );
    }
    if (liquidationPayoutCacheFailures.length === 0) {
      throw new Error(
        `expected LIQUIDATION_PAYOUT observability on liquidation; liquidatorDataPushes=[${liquidatorPushTypes.join(",") || "none"}]; liquidatorViewCacheFailures=[${liquidationManagerCacheFailures.join(" | ") || "none"}]`,
      );
    }
    console.log(
      `  [Notice] liquidation payout push failed after payout event emission: reasons=[${liquidationPayoutCacheFailures.join(" | ")}]`,
    );
  } else {
  ...
  console.log("\n✅ live-liquidation-arbitrum-sepolia PASSED\n");
}

void runWithNetworkRetry("live-liquidation-arbitrum-sepolia", main);