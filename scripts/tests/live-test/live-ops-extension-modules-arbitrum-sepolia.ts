...
async function main() {
  ...
  if (relayerHasSystemViewer) {
    ...
    if (coreStats && BigInt(coreStats.totalDegradations ?? coreStats[0] ?? 0) < 0n) {
    ...
    if (monitorStats && BigInt(monitorStats.totalDegradations ?? monitorStats[0] ?? 0) < 0n) {
    ...
    if (healthStatus && (healthStatus.isHealthy ?? healthStatus[0]) === undefined) {
  ...
  console.log("\n✅ live-ops-extension-modules-arbitrum-sepolia PASSED\n");
}

void runWithNetworkRetry("live-ops-extension-modules-arbitrum-sepolia", main);