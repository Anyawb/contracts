...
function configureLocalhostSettlementEnv(network: string) {
  ...
  process.env.SETTLEMENT_PRICE_UNITS_8 = envStr("SETTLEMENT_PRICE_UNITS_8", "100000000")!;
}
...
async function main() {
  ...
  if (network === "localhost" || network === "hardhat") {
    const prepStep: Step = {
      ...
      file: "scripts/tests/live-test/prepare-live-gates-localhost.ts",
    };
  ...
  const steps: Step[] = [
    {
      ...
      file: "scripts/tests/live-test/live-platform-baseline-arbitrum-sepolia.ts",
    },
    {
      id: "02-fee-prepaid",
      label: "fee prepaid gate",
      file: "scripts/tests/live-test/live-fee-prepaid-gate-arbitrum-sepolia.ts",
      envHints: ["FEE_PREPAID_AMOUNT_UNITS", "FEE_PREPAID_TYPE_NAME"],
    },
    {
      id: "03-fee-remaining",
      label: "fee remaining gate",
      file: "scripts/tests/live-test/live-fee-remaining-gate-arbitrum-sepolia.ts",
      envHints: ["FEE_REMAINING_AMOUNT_UNITS"],
    },
    {
      id: "04-fee-dynamic",
      label: "fee dynamic gate",
      file: "scripts/tests/live-test/live-fee-dynamic-gate-arbitrum-sepolia.ts",
