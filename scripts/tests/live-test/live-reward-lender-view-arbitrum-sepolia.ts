import {
  ...
  findRewardViewPush,
  readRewardUser,
  requireNoEasyMintSkipped,
} from "./_rewardLive";
import { ethers } from "hardhat";
import { depositCollateral, finalizeSingleMatch, repayOrder, reserveForLending } from "./_fundsFlowLive";
...
async function main() {
  ...
  const easyMintSkipped = decodeEasyMintSkipped(reward, repayReceipt);
  requireNoEasyMintSkipped(easyMintSkipped, "repay reward mint path should not skip");
  const easyMintPush = findRewardViewPush(repayPushes, "EASY_MINTED");
  if (!easyMintPush) {
    throw new Error("repay receipt should emit RewardView DataPushed(EASY_MINTED)");
  }
  ...
  console.log("\n✅ live-reward-lender-view-arbitrum-sepolia PASSED\n");
}

void runWithNetworkRetry("live-reward-lender-view-arbitrum-sepolia", main);