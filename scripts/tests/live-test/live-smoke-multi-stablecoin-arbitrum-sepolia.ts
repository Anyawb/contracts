...
import { getAssetBootstrapPriceUsd8, loadMockAssetPack, loadMockAssetPackData, type MockAssetPackAsset } from "./_mockLiveUtils";
...
function parseInlineTargets(raw: string): SmokeAssetTarget[] {
  return raw
    ...
    .filter(Boolean)
    .map((item) => {
      const [symbol, address, decimals, priceUsd8, sourceId] = item.split(":").map((part) => part.trim());
      if (!symbol || !address || !decimals || !priceUsd8) {
        throw new Error(
          `invalid MULTI_STABLECOIN_ASSETS entry=${item}. expected symbol:address:decimals:priceUsd8[:sourceId]`,
        );
      }
      return {
        symbol,
...
function parseBorrowTargetsFromPack(assets: MockAssetPackAsset[]): SmokeAssetTarget[] {
  ...
  for (const symbol of requested) {
    ...
    targets.push({
      ...
      priceUsd8: getAssetBootstrapPriceUsd8(asset),
  ...
  if (missing.length > 0 && requestedRaw) {
    throw new Error(
      `stablecoins ${missing.join(", ")} not found in mock asset pack. Refresh deployments/mock-assets.arbitrum-sepolia.json or pass MULTI_STABLECOIN_ASSETS explicitly.`,
    );
  }
  if (targets.length === 0) {
    throw new Error("no stablecoin targets found in mock asset pack. Refresh deployments/mock-assets.arbitrum-sepolia.json or pass MULTI_STABLECOIN_ASSETS explicitly.");
  }
  if (missing.length > 0) {
...
function parseCollateralTargetsFromPack(assets: MockAssetPackAsset[]): SmokeAssetTarget[] {
  ...
  if (!requestedRaw) {
    ...
    return [{
      ...
      priceUsd8: getAssetBootstrapPriceUsd8(selected),
  ...
  return requested.map((symbol) => {
    ...
    if (!asset) {
      throw new Error(
        `collateral ${symbol} not found in mock asset pack. Refresh deployments/mock-assets.arbitrum-sepolia.json or clear MULTI_COLLATERAL_SYMBOLS.`,
      );
    }
    return {
      symbol: asset.symbol,
...
async function main() {
  ...
  const originalEnv = {
    ...
    BORROW_PRICE_UNITS_8: process.env.BORROW_PRICE_UNITS_8,
    ...
    COLLATERAL_PRICE_UNITS_8: process.env.COLLATERAL_PRICE_UNITS_8,
  ...
  try {
    ...
    for (const collateralTarget of collateralTargets) {
      ...
      process.env.COLLATERAL_PRICE_UNITS_8 = collateralTarget.priceUsd8;
      ...
      for (const borrowTarget of borrowTargets) {
        ...
        process.env.BORROW_PRICE_UNITS_8 = borrowTarget.priceUsd8;
    ...
    console.log("\n✅ live-smoke-multi-stablecoin-arbitrum-sepolia PASSED\n");
  } finally {
...
main().catch((error) => {
  console.error("\n❌ live-smoke-multi-stablecoin-arbitrum-sepolia FAILED\n");
  console.error(error);
  process.exit(1);
});