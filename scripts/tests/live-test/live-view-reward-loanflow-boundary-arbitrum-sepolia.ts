...
async function main() {
  ...
  if (!afterBorrowViews.loanFlowUser || !afterRepayViews.loanFlowUser) {
    throw new Error("LoanFlowView user snapshot should be available in reward boundary script");
  }
  if (afterBorrowViews.loanFlowUser.borrowCount <= (beforeViews.loanFlowUser?.borrowCount ?? 0n)) {
    throw new Error("LoanFlowView borrowCount should increase after finalizeMatch");
  }
  if (afterRepayViews.loanFlowUser.repayCount <= (afterBorrowViews.loanFlowUser.repayCount ?? 0n)) {
  ...
  console.log("\n✅ live-view-reward-loanflow-boundary-arbitrum-sepolia PASSED\n");
}

void runWithNetworkRetry("live-view-reward-loanflow-boundary-arbitrum-sepolia", main);