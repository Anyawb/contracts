...
import { key, updateAssetPriceWithRepair } from "../core/_mockLiveUtils";
...
async function refreshPrice(ctx: FundsFlowLiveContext, asset: string, label: string) {
  ...
  const runtimeAsset = [ctx.pair.borrowAsset, ctx.pair.collateralAsset, ctx.pair.settlementAsset].find(
  ...
  if (ctx.updater) {
    try {
      ...
      const text = String(error?.shortMessage ?? error?.message ?? error?.info?.error?.message ?? "");
      const revertData = String(error?.data?.data ?? error?.data ?? error?.info?.error?.data ?? "").toLowerCase();
      const isAssetNotConfigured = text.includes("AssetNotConfigured") || revertData.startsWith("0xfaf49a83");
      if (!isAssetNotConfigured || !ctx.allowDirectOraclePriceWrite) {
        throw error;
      }
      console.log(`  [Notice] ${label}: updater asset not configured, falling back to direct PriceOracle refresh`);
    }
  }
