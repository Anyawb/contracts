...
async function main() {
  ...
  for (let index = 0; index < borrowerCount; index += 1) {
    ...
    if (!repayMintOutcome.mintedPush) {
      ...
      console.log(`  [Notice] borrower #${index + 1} repay missing RewardView DataPushed(EASY_MINTED); reason=${reason}`);
    }
    ...
    if (repayMintOutcome.hasAcceptedSkip && borrowerAfterRepay.easyBalance < consumeAmount) {
      console.log(
        `  [Notice] borrower #${index + 1} skipping consume/stake after accepted EasyMintSkipped; balance=${borrowerAfterRepay.easyBalance.toString()} reason=${repayMintOutcome.skipReason}`,
      );
