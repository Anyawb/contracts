...
function requireValidLoanFlowUser(snapshot: Awaited<ReturnType<typeof observeExtendedViews>>, stage: string) {
  ...
  if (!snapshot.loanFlowUser.isValid) {
    console.log(`  [Notice] ${stage}: LoanFlowView user snapshot is present but still invalid; skipping strict LoanFlow delta assertions`);
    return false;
  }
  return true;
}
...
async function main() {
  ...
  const afterBorrowViews = await observeExtendedViews(ctx, "reward-boundary-after-borrow");
  ...
  const afterRepayViews = await observeExtendedViews(ctx, "reward-boundary-after-repay");
  ...
  const loanFlowReadyBefore = Boolean(beforeViews.loanFlowUser?.isValid);
  ...
  if (loanFlowReadyBefore && loanFlowReadyAfterBorrow) {
    if (afterBorrowViews.loanFlowUser!.borrowCount <= beforeViews.loanFlowUser!.borrowCount) {
      throw new Error("LoanFlowView borrowCount should increase after finalizeMatch");
    }
  } else {
    console.log("  [Notice] LoanFlowView borrowCount assertion skipped because user cache was not publish-ready across the borrow edge");
  }
  if (loanFlowReadyAfterBorrow && loanFlowReadyAfterRepay) {
    ...
    console.log("  [Notice] LoanFlowView repayCount assertion skipped because user cache was not publish-ready across the repay edge");
  }
