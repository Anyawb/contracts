...
import { networkSlug, withNonceManagedSigner } from "./_mockLiveUtils";
...
function getMnemonic() {
  return envStr("LIVE_FRESH_BORROWER_MNEMONIC")?.trim()
    ...
    || loadOrCreateDefaultFreshBorrowerMnemonic();
}
...
function resolveDefaultFreshBorrowerDir() {
  return path.resolve(process.cwd(), "scripts", "tests", "logs", "fresh-borrowers", resolveFreshBorrowerNetworkSlug());
}
