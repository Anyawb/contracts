...
function resolveOrderReadConvergenceTimeoutMs() {
...
function resolveOrderReadConvergencePollMs() {
...
export async function waitForFreshOrderReadRecovery(params: {
  ...
  const timeoutMs = params.timeoutMs ?? resolveOrderReadConvergenceTimeoutMs();
  const pollMs = params.pollMs ?? resolveOrderReadConvergencePollMs();
...
export async function ensureFundsFlowPrices(ctx: FundsFlowLiveContext) {
  ...
  const ensureOracleReadable = async (
    ...
    const needsConfigure = !config.isActive || configuredDecimals !== BigInt(item.asset.decimals) || sourceId.length === 0;
    ...
    if (needsConfigure) {
      ...
      console.log(`[PriceRepair] configured ${item.asset.symbol} directly in PriceOracle sourceId=${item.asset.sourceId} decimals=${item.asset.decimals}`);
    }
    ...
    const [price, blockNumber] = (await ctx.priceOracle.getPrice(item.asset.address)) as [bigint, bigint, bigint];
    if (price <= 0n || blockNumber <= 0n) {
      throw new Error(`PriceOracle repair did not produce a readable price for ${item.asset.symbol}`);
    }
  };
...
export async function assignFreshBorrower(
  ...
  const maxAttempts = Math.max(1, Number(envStr("LIVE_FRESH_BORROWER_MAX_ATTEMPTS") ?? "8"));
...
export async function ensureApprovalNativeTopUp(params: {
  ...
  const approvalGasPrice = feeData.maxFeePerGas ?? feeData.gasPrice ?? 50_000_000n;
...
export async function depositCollateralAsset(
  ...
  const feeData = await ethers.provider.getFeeData();
  const depositGasPrice = feeData.maxFeePerGas ?? feeData.gasPrice ?? 50_000_000n;
...
export async function reserveForLendingAsset(
  ...
  try {
    ...
    if (/ERC20InsufficientBalance/i.test(explained)) {
      ...
      throw new Error(`reserveForLending staticCall reverted: ${explained}`);
    }
  }
  const reserveNativeFloorWei = ethers.parseEther(envStr("LIVE_FUNDS_FLOW_RESERVE_NATIVE_MIN_ETH") ?? "0.001");
  const reserveSponsorReserveWei = ethers.parseEther(
    envStr("LIVE_FUNDS_FLOW_RESERVE_SPONSOR_RESERVE_ETH") ?? resolveFreshBorrowerSponsorReserveEth(),
  );
  const buildReserveTxRequest = async () => {
    ...
    const txRequest: any = {
      ...
      data: populatedReserveTx.data,
      value: populatedReserveTx.value ?? 0n,
      gasLimit: reserveGasLimit,
    };
    if (reserveFeeData.maxFeePerGas != null || reserveFeeData.maxPriorityFeePerGas != null) {
      const maxPriorityFeePerGas = reserveFeeData.maxPriorityFeePerGas ?? reserveFeeData.gasPrice ?? 0n;
      const maxFeePerGas = reserveFeeData.maxFeePerGas ?? maxPriorityFeePerGas;
      txRequest.maxFeePerGas = maxFeePerGas;
      txRequest.maxPriorityFeePerGas = maxPriorityFeePerGas > maxFeePerGas ? maxFeePerGas : maxPriorityFeePerGas;
    } else if (reserveFeeData.gasPrice != null) {
      txRequest.gasPrice = reserveFeeData.gasPrice;
    } else {
      txRequest.gasPrice = 50_000_000n;
    }
    const feeCeilingWei = txRequest.gasPrice != null
      ? reserveGasLimit * BigInt(txRequest.gasPrice)
      : reserveGasLimit * BigInt(txRequest.maxFeePerGas ?? 0n);
    const desiredBalanceWei = feeCeilingWei
      + BigInt(txRequest.value ?? 0n)
      + ethers.parseEther(envStr("LIVE_FUNDS_FLOW_RESERVE_NATIVE_RESERVE_ETH") ?? "0.00005");
    return {
      txRequest,
      reserveGasLimit,
      feeCeilingWei,
      desiredBalanceWei: desiredBalanceWei > reserveNativeFloorWei ? desiredBalanceWei : reserveNativeFloorWei,
    };
  };

  let reservePrepared = await buildReserveTxRequest();
  const lenderNativeBeforeReserve = await ethers.provider.getBalance(ctx.lender.address);
  console.log(
    `[ReserveGas] ${ctx.label}/${symbol} before=${ethers.formatEther(lenderNativeBeforeReserve)} target=${ethers.formatEther(reservePrepared.desiredBalanceWei)} gasLimit=${reservePrepared.reserveGasLimit.toString()} feeCeilingWei=${reservePrepared.feeCeilingWei.toString()}`,
  );
  await ensureRecoverableNativeTopUp({
    label: `${ctx.label}-${symbol.toLowerCase()}-reserve-native-gas`,
    target: ctx.lender,
    sponsors: buildNativeTopUpSponsors([ctx.lender.address], ctx.relayer, ctx.borrower, ctx.viewer),
    desiredBalanceWei: reservePrepared.desiredBalanceWei,
    reserveWei: reserveSponsorReserveWei,
    failOnShortfall: true,
  });
  let reserveTx;
  try {
    reserveTx = await ctx.lender.sendTransaction(reservePrepared.txRequest);
  } catch (error: any) {
    const explained = explainRevert(error, [ctx.vbl.interface, token.interface, LIVE_FLOW_ERROR_INTERFACE, ERC20_ERROR_INTERFACE].filter(Boolean as any));
    if (!/ERC20InsufficientBalance/i.test(explained)) {
      throw error;
    ...
    await ensureLenderReserveBalance();
    reservePrepared = await buildReserveTxRequest();
    await ensureRecoverableNativeTopUp({
      label: `${ctx.label}-${symbol.toLowerCase()}-reserve-native-gas-retry`,
      target: ctx.lender,
      sponsors: buildNativeTopUpSponsors([ctx.lender.address], ctx.relayer, ctx.borrower, ctx.viewer),
      desiredBalanceWei: reservePrepared.desiredBalanceWei,
      reserveWei: reserveSponsorReserveWei,
...
export async function repayOrderAsset(
  ...
  try {
    ...
    const feeData = await ethers.provider.getFeeData();
    const repayGasPrice = feeData.maxFeePerGas ?? feeData.gasPrice ?? 50_000_000n;
