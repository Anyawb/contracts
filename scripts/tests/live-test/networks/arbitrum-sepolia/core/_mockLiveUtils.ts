...
export async function withNonceManagedSigner<T extends NonceManagedSigner>(signer: T): Promise<T> {
  ...
  const applyNetworkFeeFloor = (tx: any) => {
    ...
    const minPriorityFeePerGas = 100_000_000n;
    if (tx?.gasPrice != null) {
  ...
  const sendTransaction = async (tx: any) => {
    ...
    try {
      ...
      const initialNonce = requestedNonce ?? sharedState.nextNonce ?? await loadSuggestedNonce();
      try {
        return await sendWithNonce(initialNonce);
      } catch (error: any) {
        ...
        return await sendWithNonce(refreshedNonce);
      }
    } finally {
      releaseQueue();
    }
