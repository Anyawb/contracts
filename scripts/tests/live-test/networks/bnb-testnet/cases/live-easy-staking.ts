...
async function main() {
  ...
  try {
    ...
    requireEasyMintedOrAcceptedSkip(repayOutcome, "EasyStaking repay mint path should mint or accepted-skip");
    ...
    if (!repayOutcome.mintedPush) {
      if (easyBalanceBefore !== 0n) {
        throw new Error(
          `EasyStaking accepted-skip path expects zero EASY balance, got ${easyBalanceBefore.toString()}`,
        );
      }
      console.log(`  [Notice] EasyStaking accepted skip: ${repayOutcome.skipReason}; skip stake/unstake assertions`);
