...
type LiquidationExecutionState = {
  debt: bigint;
...
async function sendPopulatedTxWithNetworkRetry(params: {
  ...
  for (let i = 0; i < multipliers.length; i += 1) {
    ...
    try {
      ...
      if (i === 0) {
        console.log(`  [Diag.LiquidationGas] ${params.label} estimate=${estimate.toString()} limit=${gasLimit.toString()}`);
      } else {
...
async function waitForLiquidationExecutionWindow(params: {
  ...
  let lastState: LiquidationExecutionState = {
    debt: 0n,
    reducibleDebt: 0n,
  ...
  while (Date.now() < deadline) {
    ...
    try {
      ...
      staticCallReady = false;
      lastStaticCallError = error instanceof Error ? error.message : String(error);
    }
...
async function waitForPostLiquidationDebtConvergence(params: {
  lendingEngine: any;
  borrower: string;
  debtAsset: string;
  expectedDebt: bigint;
  expectedReducibleDebt: bigint;
  timeoutMs?: number;
  pollMs?: number;
}) {
  const timeoutMs = Math.max(1_000, params.timeoutMs ?? 20_000);
  const pollMs = Math.max(200, params.pollMs ?? 1_500);
  const deadline = Date.now() + timeoutMs;

  let lastDebt = -1n;
  let lastReducibleDebt = -1n;
  let attempt = 0;
  ...
  while (Date.now() < deadline) {
    ...
    lastReducibleDebt = BigInt(reducibleDebt);
    console.log(
      `  [Poll.PostLiquidationDebt] attempt=${attempt} debt=${lastDebt.toString()} reducibleDebt=${lastReducibleDebt.toString()} expectedDebt=${params.expectedDebt.toString()} expectedReducibleDebt=${params.expectedReducibleDebt.toString()}`,
    );

    if (lastDebt === params.expectedDebt && lastReducibleDebt === params.expectedReducibleDebt) {
      return {
        debtAfter: lastDebt,
        reducibleDebtAfter: lastReducibleDebt,
      };
    }
  ...
  throw new Error(
    `post-liquidation debt did not converge within ${timeoutMs}ms: debt=${lastDebt.toString()} reducibleDebt=${lastReducibleDebt.toString()} expectedDebt=${params.expectedDebt.toString()} expectedReducibleDebt=${params.expectedReducibleDebt.toString()}`,
  );
}
...
async function getLogsAdaptive(params: {
  ...
  try {
    ...
    if (params.fromBlock === params.toBlock) {
      console.log(
        `  [CandidateSearch] skip dense block=${params.fromBlock.toString()} due to getLogs limit (${error instanceof Error ? error.message : String(error)})`,
      );
      return [];
    }
    ...
    if (span <= 8n) {
      const logs: any[] = [];
      for (let block = params.fromBlock; block <= params.toBlock; block += 1n) {
        try {
          const blockLogs = await ethers.provider.getLogs({
            address: params.address,
            fromBlock: block,
            toBlock: block,
            topics: [params.topic0],
          ...
          if (!isGetLogsRangeLimitError(blockError)) {
            throw blockError;
          }
          console.log(
            `  [CandidateSearch] skip dense block=${block.toString()} due to getLogs limit (${blockError instanceof Error ? blockError.message : String(blockError)})`,
          );
        }
      }
      return logs;
...
async function main() {
  ...
  const allowAutoSeedOnMockNetworks = envBool(
    "LIQUIDATION_AUTO_SEED_ON_MOCK_NETWORKS",
  ...
  let orderBefore: Awaited<ReturnType<typeof getOrderForView>> | null = null;
  let orderSource: LiquidationOrderSource | null = null;
  let configuredOrderValid = true;
  let allowRiskTriggeredOrder = allowRiskTriggeredConfiguredOrder;
  const seedFreshLiquidationOrder = async () => {
    const seeded = await seedLiquidatableOrder({
      label: "Seed Liquidatable Live Liquidation Order",
      noticeLabel: "using fresh liquidation borrower",
      collateralAmountUnitsDefault: "10",
      borrowAmountUnitsDefault: "1200",
    });
    ...
    orderSource = "fresh";
    configuredOrderValid = true;
    allowRiskTriggeredOrder = seeded.acceptRiskTriggeredOrder;
    console.log(
      `  [LiquidationSeed] auto-seeded isolated liquidation orderId=${orderId.toString()} borrower=${seeded.borrower} source=${seeded.source}`,
    );
  };

  if (configuredSeedLogFile && !configuredOrderId) {
    ...
    if (!orderBefore.borrower || orderBefore.borrower === ethers.ZeroAddress || !orderBefore.asset || orderBefore.asset === ethers.ZeroAddress) {
      ...
      console.log(
        `  [Notice] seeded liquidation order is stale or missing on current registry: orderId=${orderId.toString()} registry=${ctx.registryAddr} borrower=${String(orderBefore.borrower)} asset=${String(orderBefore.asset)}`,
      );
    } else {
      console.log(
        `  [LiquidationSeed] using seeded order from ${configuredSeedLogFile}: orderId=${orderId.toString()} borrower=${seeded.borrower}`,
      );
    }
  } else if (configuredOrderId) {
    ...
    orderSource = "configured";
  } else if (network.name !== "localhost" && network.name !== "hardhat" && allowAutoSeedOnMockNetworks) {
    await seedFreshLiquidationOrder();
  } else {
  ...
  if ((configuredOrderId && (!configuredOrderValid || !readinessResult?.readiness.liquidatable))) {
    if (allowAutoSeedOnMockNetworks) {
      await seedFreshLiquidationOrder();
      readinessResult = await readLiquidationReadiness({
        ctx,
        orderId: orderId!,
  ...
  if (orderId === null || orderBefore === null || orderSource === null) {
    throw new Error("liquidation order state unresolved after selection");
  }
  const currentOrderId = orderId;
  const currentOrderBefore = orderBefore;

  let { readiness, seizableAssets, seizableAmounts } = readinessResult;

  if (readiness.currentBlock <= currentOrderBefore.maturity && !(allowRiskTriggeredOrder && readiness.liquidatable)) {
    throw new Error(
      `liquidation order is not overdue: currentBlock=${readiness.currentBlock.toString()} maturity=${currentOrderBefore.maturity.toString()}`,
    );
  }
  if (readiness.currentBlock <= currentOrderBefore.maturity && allowRiskTriggeredOrder && readiness.liquidatable) {
    console.log(
      `  [Notice] configured mock-suite liquidation order is not overdue but risk-triggered execution is allowed: orderId=${currentOrderId.toString()} maturity=${currentOrderBefore.maturity.toString()} currentBlock=${readiness.currentBlock.toString()}`,
    );
  }

  const debtAsset = currentOrderBefore.asset;
  ...
  await waitForLiquidationExecutionWindow({
    settlementManager: settlementManager.connect(ctx.relayer),
    lendingEngine,
    borrower: currentOrderBefore.borrower,
    debtAsset,
  ...
  const settleOrLiquidatePopulated = await settlementManager
    .connect(ctx.relayer)
    .settleOrLiquidate.populateTransaction(currentOrderId);
  const liquidationReceipt = await sendPopulatedTxWithNetworkRetry({
    ...
    label: "settleOrLiquidate-liquidation",
    multipliersBps: [13_000n, 16_000n, 20_000n],
  });

  await waitForPostWriteOrderReadConvergence(ctx, currentOrderId, "settleOrLiquidate", liquidationReceipt.blockNumber);

  const expectedDebtAfter = debtBefore - reducibleBefore;
  const { debtAfter, reducibleDebtAfter } = await waitForPostLiquidationDebtConvergence({
    lendingEngine,
    borrower: currentOrderBefore.borrower,
    debtAsset,
    expectedDebt: expectedDebtAfter,
    expectedReducibleDebt: 0n,
    timeoutMs: Number(process.env.LIVE_LIQUIDATION_POST_TX_TIMEOUT_MS ?? "20000"),
    pollMs: Number(process.env.LIVE_LIQUIDATION_POST_TX_POLL_MS ?? "1500"),
  });

  const orderAfter = await getOrderForView(ctx, currentOrderId);
  const explicitOrderStatus = await getOrderLifecycleStatus(ctx, currentOrderId);
  const loanNftStatus = await getLoanNftViewStatus(ctx, currentOrderBefore.borrower, currentOrderId);
  const hasShortfall = (await settlementManager.hasActiveShortfall(currentOrderId)) as boolean;
  const terminalStatuses = readiness.currentBlock > currentOrderBefore.maturity
    ? { clean: LOAN_STATUS.Defaulted, withShortfall: LOAN_STATUS.DefaultedWithShortfall }
    : { clean: LOAN_STATUS.Liquidated, withShortfall: LOAN_STATUS.LiquidatedWithShortfall };
  ...
  if (debtAfter !== expectedDebtAfter) {
    throw new Error(
      `debt delta mismatch: before=${debtBefore.toString()} after=${debtAfter.toString()} reducible=${reducibleBefore.toString()}`,
    );
  }
  if (reducibleDebtAfter !== 0n) {
    throw new Error(`reducible debt should be zero after liquidation: ${reducibleDebtAfter.toString()}`);
  }
  if (orderAfter.borrower.toLowerCase() !== currentOrderBefore.borrower.toLowerCase()) {
    throw new Error("order borrower changed unexpectedly after liquidation");
  }
  if (explicitOrderStatus !== terminalStatuses.clean && explicitOrderStatus !== terminalStatuses.withShortfall) {
    throw new Error(
      `unexpected order terminal status after liquidation: status=${formatLoanStatus(explicitOrderStatus)} expected=${formatLoanStatus(terminalStatuses.clean)}|${formatLoanStatus(terminalStatuses.withShortfall)}`,
    );
  }
  expectBigintEq("LoanNFTView status after liquidation", loanNftStatus, explicitOrderStatus);
  if (explicitOrderStatus === terminalStatuses.withShortfall) {
    if (!hasShortfall) {
      throw new Error("with-shortfall order status requires an active shortfall ledger");
    }
    const shortfallLedger = await settlementManager.getShortfallLedger(currentOrderId);
    expectBigintEq("shortfall ledger remaining debt after liquidation", BigInt(shortfallLedger.remainingDebt), debtAfter);
  } else {
    if (hasShortfall) {
      throw new Error("clean liquidation/default status should not leave an active shortfall ledger");
    }
    expectBigintEq("debt after clean liquidation", debtAfter, 0n);
  }
  console.log(
    `  [StateMachine] orderStatus=${formatLoanStatus(explicitOrderStatus)} loanNftStatus=${formatLoanStatus(loanNftStatus)}`,
  );
  ...
  const allDataPushLogs: ParsedDataPushLog[] = (liquidationReceipt.logs ?? []).flatMap((log: any) => {
