...
async function main() {
  ...
  const feeRouter = (await ethers.getContractAt(
    [
      "function batchDistribute(address token,uint256[] amounts,bytes32[] feeTypes)",
    ],
    ctx.feeRouterAddr,
  )) as any;

  const guaranteeFundManager =
    ctx.guaranteeFundAddr && String(ctx.guaranteeFundAddr).toLowerCase() !== ethers.ZeroAddress.toLowerCase()
  ...
  if (unauthorizedSigner) {
    ...
    await expectRevert("VaultRouter pause gate should revert", async () =>
      vaultRouter.connect(unauthorizedSigner).pause.staticCall(),
    );
    await expectRevert("VaultRouter unpause gate should revert", async () =>
      vaultRouter.connect(unauthorizedSigner).unpause.staticCall(),
    );
    await expectRevert("FeeRouter batchDistribute gate should revert", async () =>
      feeRouter
        .connect(unauthorizedSigner)
        .batchDistribute.staticCall(ctx.borrowAssetAddr, [ethers.parseUnits("1", ctx.borrowDecimals)], [key("DEPOSIT")]),
    );
    if (guaranteeFundManager) {
      await expectRevert("GuaranteeFundManager releaseGuarantee direct gate should revert", async () =>
        guaranteeFundManager
          .connect(unauthorizedSigner)
          .releaseGuarantee
          .staticCall(ctx.borrower.address, ctx.borrowAssetAddr, 1n),
      );
      await expectRevert("GuaranteeFundManager forfeitGuarantee direct gate should revert", async () =>
        guaranteeFundManager
          .connect(unauthorizedSigner)
          .forfeitGuarantee
          .staticCall(ctx.borrower.address, ctx.borrowAssetAddr, ctx.relayer.address),
      );
    } else {
      console.log("  [Notice] GuaranteeFundManager is not registered; direct entry gate checks skipped");
    }
    if (featureKey) {
