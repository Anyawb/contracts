...
import { fmtErr, getReadCaller, key, loadMockAssetPack, requireCode } from "../core/_mockLiveUtils";
...
async function main() {
  ...
  if (!viewerRewardCachesWarm) {
    ...
    if (viewerRewardCachePresent) {
      console.log("Info: RewardView viewer caches are stale for the current viewer, but readable data exists");
    } else {
    ...
    try {
      ...
      if (!valid) {
        if (hasCacheBlock(blockNumber)) {
          console.log("Info: HealthView self cache is stale for the current viewer, but data exists");
        } else {
    ...
    try {
      ...
      if (!valid) {
        if (hasCacheBlock(updateBlock)) {
          console.log("Info: PositionView collateral cache is stale for the current viewer, but data exists");
        } else {
      ...
      if (!borrowValid) {
        if (hasCacheBlock(borrowUpdateBlock)) {
          console.log("Info: PositionView borrow-asset cache is stale for the current viewer, but data exists");
        } else {
