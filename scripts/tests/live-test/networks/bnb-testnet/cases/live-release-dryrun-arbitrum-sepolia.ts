...
function resolveWrapperNetwork() {
  ...
  if (configured && configured !== "arbitrumSepolia") {
    throw new Error(`live-release-dryrun-arbitrum-sepolia expects LIVE_RELEASE_NETWORK=arbitrumSepolia, got ${configured}`);
  }
  ...
  const readOnlySteps: ScriptStep[] = [
    {
      ...
      file: "scripts/tests/live-test/networks/arbitrum-sepolia/live-asset-precheck.ts",
    },
    {
      id: "02-preflight",
      label: "protocol preflight",
      file: "scripts/tests/live-test/networks/arbitrum-sepolia/live-preflight.ts",
    },
  ];
