...
async function runRecycleRecoveryBranch() {
  ...
  if (!repayOutcome.mintedPush && repayOutcome.hasAcceptedSkip) {
    console.log(
      `  [Notice] recycle recovery accepted skip: ${repayOutcome.skipReason}; skip EASY recycle assertions for this round`,
    );
    return;
  }
  ...
  if (!repayOutcome.mintedPush) {
    throw new Error(
      `reward recycle recovery live requires EASY_MINTED or accepted skip; got skipReason=${String(repayOutcome.skipReason ?? "none")}`,
    );
  }
