...
/// @dev Borrow always reverts with "LE fail" for negative test scenarios.
contract BadLendingEngine is ILendingEngineBasic {
    function borrow(
        ...
        amount;
        collateralAdded;
        termDays; // silence unused parameters
        revert("LE fail");
    }
    ...
    function repay(address user, address asset, uint256 amount) external pure {
        user;
        asset;
        amount; // silence unused parameters
    }
    ...
    function getDebt(
        ...
        user;
        asset; // silence unused parameters
        return 0;
    }

    function getTotalDebtByAsset(
        address asset
    ) external pure returns (uint256 totalDebt) {
    ...
    function getUserDebtAssets(
        address user
    ) external pure returns (address[] memory assets) {
        user; // silence unused parameter
    ...
    function getReducibleDebtAmount(
        address user,
        address asset
    ) external pure returns (uint256 reducibleAmount) {
        user;
        asset;
        return 0;
    }
