...
contract MockPositionViewRoleGated {
    ...
    function getUserPositionWithMeta(address user, address asset)
        external
        view
        returns (uint256 collateral_, uint256 debt_, bool isValid, uint256 blockNumber, uint64 version_)
    {
        if (msg.sender != user) {
            bool ok =
                ViewAccessLib.hasRole(registryAddr, ActionKeys.ACTION_ADMIN, msg.sender)
                    || ViewAccessLib.hasRole(registryAddr, ActionKeys.ACTION_VIEW_USER_DATA, msg.sender);
