...
contract SystemView is Initializable, UUPSUpgradeable, ViewVersioned {
    ...
    function getModule(
        bytes32 key
    ) external view onlyValidRegistry onlyViewRole returns (address) {
    ...
    function getNamedModule(
        ...
        address legacyModuleAddr = Registry(_registryAddr).getModule(
            keccak256(bytes(name))
        );
        if (legacyModuleAddr == address(0))
            revert SystemView__UnknownModuleName();
     ...
     * @return moduleAddr Module address, or address(0) if not configured.
     */
    function getNamedModuleOptional(
        string calldata name
    ) external view onlyValidRegistry onlyViewRole returns (address) {
        bytes32 key = ModuleKeys.getModuleKeyFromString(name);
        if (key != bytes32(0)) {
    ...
    function routePrice()
        ...
        onlyValidRegistry
        onlyViewRole
        returns (RouteHint memory routeHint_)
    {
        routeHint_.primaryRoute = RouteInfo({
            moduleKey: ModuleKeys.KEY_VALUATION_ORACLE_VIEW,
            moduleAddr: Registry(_registryAddr).getModule(
                ModuleKeys.KEY_VALUATION_ORACLE_VIEW
            )
        });
    ...
    function routeStatistics()
        ...
        onlyValidRegistry
        onlyViewRole
        returns (RouteInfo memory routeInfo_)
    {
