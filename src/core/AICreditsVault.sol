...
import { ZeroAddress, NotAContract, InvalidCaller } from "../errors/StandardErrors.sol";

/// @title AICreditsVault
/// @notice AI credits on-chain balance SSOT : buy credits + batch settle deductions.
/// @dev Per docs/Usage-Guide/AI-Credits-Billing-Guide.md:
///      - High-frequency per-request charging is OFF-CHAIN (usage ledger).
///      - On-chain only tracks auditable creditsBalance and batch settlements.
contract AICreditsVault is Initializable, UUPSUpgradeable, ReentrancyGuardUpgradeable {
    using SafeERC20 for IERC20;

    address private _registryAddr;

    /// @dev credits are natural numbers: 1 = 1 AI call.
    mapping(bytes32 tenantId => mapping(address user => uint256 credits)) private _creditsBalance;

    /// @dev Purchase/exchange idempotency: (tenantId,user,clientOrderId) can only apply once.
    mapping(bytes32 tenantId => mapping(address user => mapping(bytes32 clientOrderId => bool used))) private _usedClientOrderId;

    /// @dev Settlement idempotency: each settlementBatchId can only apply once.
    mapping(bytes32 settlementBatchId => bool applied) private _appliedSettlementBatch;
    ...
    event PricePerCreditUpdated(address indexed payToken, uint256 oldPricePerCredit, uint256 newPricePerCredit);
    ...
    function isClientOrderIdUsed(bytes32 tenantId, address user, bytes32 clientOrderId) external view returns (bool) {
        return _usedClientOrderId[tenantId][user][clientOrderId];
    }

    function isSettlementBatchApplied(bytes32 settlementBatchId) external view returns (bool) {
    ...
    function setPricePerCredit(address payToken, uint256 newPricePerCredit) external nonReentrant {
    ...
    function buyCredits(
        ...
        if (_usedClientOrderId[tenantId][msg.sender][clientOrderId]) revert InvalidCaller();
        ...
        emit CreditsPurchased(tenantId, msg.sender, payToken, payAmount, credits, clientOrderId, block.number);
    }
    ...
    function settleBatch(
        ...
        if (users.length == 0 || users.length != creditsUsed.length) revert InvalidCaller();
        ...
        emit CreditsSettled(tenantId, settlementBatchId, users.length, total, merkleRoot, block.number);
    }
