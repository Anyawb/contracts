...
contract LendingEngine is Initializable, PausableUpgradeable, UUPSUpgradeable {
     ...
     */
    event RegistryDynamicModuleKeyUpdated(
    ...
    function initialize(address initialRegistryAddr) external initializer {
        if (initialRegistryAddr == address(0))
            revert LendingEngine__ZeroAddress();
     ...
     */
    function setRegistryDynamicModuleKey(
        address dynamicModuleKeyAddr
    ) external onlyValidRegistry {
        _requireRole(ActionKeys.ACTION_SET_PARAMETER, msg.sender);
        if (dynamicModuleKeyAddr == address(0))
            revert LendingEngine__ZeroAddress();

        address oldAddr = address(_registryDynamicModuleKey);
        _registryDynamicModuleKey = IRegistryDynamicModuleKey(
            dynamicModuleKeyAddr
        );
    ...
    function createLoanOrder(
        ...
        {
            address loanFlowPM = IRegistry(_registryAddr).getModule(
                ModuleKeys.KEY_LOAN_FLOW_PUSH_MANAGER
            );
            if (loanFlowPM != address(0) && loanFlowPM.code.length != 0) {
                try
                    ILoanFlowPushManagerNotify(loanFlowPM).notifyBorrow(
                        order.borrower,
                        order.asset,
                        order.principal,
                        orderId
        ...
        // Notify RewardManager after order creation (best-effort).
        // - Single supported callback: order-based (orderId/maturity/outcome).
        address rewardManagerBorrow = IRegistry(_registryAddr)
            .getModuleOrRevert(ModuleKeys.KEY_RM);
        bool rewardNotified = false;
        try
            IRewardManagerByOrderWithLender(rewardManagerBorrow)
        ...
        if (!rewardNotified) {
            try
                IRewardManagerByOrder(rewardManagerBorrow).onLoanEventByOrder(
                    order.borrower,
                    orderId,
                    order.principal,
                    maturityBlock,
    ...
    function repay(
        ...
        if (_repayAmount == 0 || _repayAmount > totalDue - ord.repaidAmount)
        ...
        // - Use a principal-first mapping: reduce principal by the delta of repaid principal.
        {
            ...
            uint256 principalBefore = repaidBefore < principal
                ? repaidBefore
                : principal;
            uint256 principalAfter = ord.repaidAmount < principal
                ? ord.repaidAmount
                : principal;
            ...
            if (principalDelta > 0) {
                ...
                IVaultCoreRepayFor(vaultCore).repayFor(
                    ord.borrower,
                    ord.asset,
                    principalDelta
                );
        ...
        // MUST NOT revert repayment (ledger SSOT already synced via VaultCore.repayFor).
        {
            ...
            if (loanFlowPM != address(0) && loanFlowPM.code.length != 0) {
                try
                    ILoanFlowPushManagerNotify(loanFlowPM).notifyRepay(
                        ord.borrower,
                        ord.asset,
                        _repayAmount,
                        orderId,
        ...
        bool isOnTime = (nowBlock + _ON_TIME_WINDOW_BLOCKS >= ord.maturity) &&
        ...
        if (isFullyRepaid) {
            address rewardManagerRepay = IRegistry(_registryAddr)
                .getModuleOrRevert(ModuleKeys.KEY_RM);

            // Order-based outcome.
            IRewardManagerByOrder.LoanEventOutcome outcome;
            if (isOnTimeAndFullyRepaid) {
                outcome = IRewardManagerByOrder
                    .LoanEventOutcome
                    .RepayOnTimeFull;
            } else {
            ...
            try
                IRewardManagerByOrderWithLender(rewardManagerRepay)
                    .onLoanEventByOrderWithLender(
                        ord.borrower,
                        ord.lender,
                        ord.asset,
            ...
            if (!repayNotified) {
                try
                    IRewardManagerByOrder(rewardManagerRepay)
                        .onLoanEventByOrder(
                            ord.borrower,
                            orderId,
                            ord.principal,
    ...
    function getUserLoanCountForView(
        address user
    ) external view onlyValidRegistry returns (uint256 count) {
    ...
    function getFailedFeeAmountForView(
        uint256 orderId
    ) external view onlyValidRegistry returns (uint256 feeAmount) {
        _requireRole(ActionKeys.ACTION_VIEW_SYSTEM_DATA, msg.sender);
    ...
    function getNftRetryCountForView(
        uint256 orderId
    ) external view onlyValidRegistry returns (uint256 retryCount) {
        _requireRole(ActionKeys.ACTION_VIEW_SYSTEM_DATA, msg.sender);
    ...
    function isMatchEngineForView(
        ...
        returns (address acmAddr) {
            return
                IAccessControlManager(acmAddr).hasRole(
                    ActionKeys.ACTION_ORDER_CREATE,
                    account
                );
        } catch {
            return false;
    ...
    function _mintNftWithRetry(
        ...
        unchecked {
            for (uint256 i = 0; i <= _MAX_RETRY_COUNT; i++) {
                try _loanNft.mintLoanCertificate(borrower, meta) returns (
                    uint256 _tokenId
                ) {
                    tokenId = _tokenId;
    ...
    function _distributeFeeWithFallback(
        uint256 orderId,
        address asset,
        uint256 feeAmount
    ) internal {
    ...
    function _updateModuleAddresses() internal {
        ...
        try
            IRegistry(_registryAddr).getModuleOrRevert(ModuleKeys.KEY_LOAN_NFT)
        returns (address loanNFTAddr) {
        ...
        try
            IRegistry(_registryAddr).getModuleOrRevert(ModuleKeys.KEY_FR)
        returns (address feeRouterAddr) {
            if (
                feeRouterAddr != address(0) &&
                feeRouterAddr != address(_feeRouter)
            ) {
    ...
    function _isAllowedDuration(
        ...
        return (durationBlocks == _DUR_5D_BLOCKS ||
            durationBlocks == _DUR_10D_BLOCKS ||
            durationBlocks == _DUR_15D_BLOCKS ||
            durationBlocks == _DUR_30D_BLOCKS ||
            durationBlocks == _DUR_60D_BLOCKS ||
            durationBlocks == _DUR_90D_BLOCKS ||
            durationBlocks == _DUR_180D_BLOCKS ||
            durationBlocks == _DUR_360D_BLOCKS);
    }

    /// @dev Return true if duration is a long duration (>= 90d baseline; blocks-based).
    function _isLongDuration(
        uint256 durationBlocks
    ) internal pure returns (bool) {
        return (durationBlocks == _DUR_90D_BLOCKS ||
            durationBlocks == _DUR_180D_BLOCKS ||
            durationBlocks == _DUR_360D_BLOCKS);
    }
    ...
    function _authorizeUpgrade(address newImplementation) internal override {
        ...
        if (newImplementation.code.length == 0)
            revert LendingEngine__InvalidImplementation();
