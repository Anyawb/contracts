...
describe('VaultView – 缓存一致性与回退路径', function () {
  async function deployFixture() {
    ...
    const vaultView = (await VaultViewFactory.deploy()) as VaultView;
    ...
    await vaultView.connect(admin).refreshModuleCache();

    return { admin, user, registry, acm, cm, le, priceOracle, vaultView };
  }

  describe('缓存有效路径', function () {
    it('缓存有效时应返回缓存值并标记有效', async function () {
      const { admin, user, cm, le, vaultView } = await loadFixture(deployFixture);
      ...
      // 管理员同步到缓存
      await vaultView.connect(admin).syncUserPositionFromLedger(userAddr, asset);

      const [collateral, debt, isValid] = await vaultView.getUserPositionWithValidity(userAddr, asset);
    ...
    it('同步后 isUserCacheValid 应为 true', async function () {
      ...
      expect(await vaultView.isUserCacheValid(userAddr)).to.equal(true);
    });
  ...
  describe('缓存过期回退到账本', function () {
    ...
    it('缓存过期后 isUserCacheValid 应为 false', async function () {
      ...
      expect(await vaultView.isUserCacheValid(userAddr)).to.equal(false);
    });

    it('从未缓存过时应直接回退到账本且标记无效', async function () {
      const { user, cm, le, vaultView } = await loadFixture(deployFixture);
      const userAddr = user.address;
      const asset = ethers.Wallet.createRandom().address;
  ...
  describe('管理员手动同步', function () {
    it('syncUserPositionFromLedger 应刷新缓存并发出事件', async function () {
      ...
      await expect(
        vaultView.connect(admin).syncUserPositionFromLedger(userAddr, asset)
      ).to.emit(vaultView, 'CacheSyncedFromLedger').withArgs(userAddr, asset, 42, 24, anyValue);
    ...
    it('非管理员调用 syncUserPositionFromLedger 应被拒绝', async function () {
      const { user, vaultView } = await loadFixture(deployFixture);
      const asset = ethers.Wallet.createRandom().address;
      await expect(
        vaultView.connect(user).syncUserPositionFromLedger(user.address, asset)
  ...
  describe('业务白名单与模块缓存', function () {
    it('缓存失效时白名单地址调用应自动刷新并放行', async function () {
      const { admin, user, vaultView, registry } = await loadFixture(deployFixture);
      ...
      // 调用应自动刷新缓存并允许白名单地址推送
      await expect(
        vaultView.connect(biz).pushUserPositionUpdate(userAddr, asset, 11, 22)
      ).to.emit(vaultView, 'UserPositionUpdated').withArgs(userAddr, asset, 11, 22, anyValue);
    });
    ...
    it('非白名单地址调用应被拒绝', async function () {
      ...
      await expect(
        vaultView.connect(user).pushUserPositionUpdate(user.address, asset, 1, 1)
  ...
  describe('模块缓存管理', function () {
    it('refreshModuleCache 应更新模块缓存并发出事件', async function () {
      const { admin, registry, vaultView } = await loadFixture(deployFixture);
      const [, , , beforeTs] = await vaultView.getCacheStats();
      ...
      await expect(vaultView.connect(admin).refreshModuleCache()).to.emit(
        vaultView,
        'ModuleCacheRefreshed'
      );

      const [, , , afterTs] = await vaultView.getCacheStats();
      expect(afterTs).to.be.gt(beforeTs);
      // 刷新后模块缓存应有效
      expect(await vaultView.isModuleCacheValid()).to.equal(true);
    });

    it('模块缓存过期后 isModuleCacheValid 应为 false', async function () {
      const { vaultView } = await loadFixture(deployFixture);
      await time.increase(3601);
      expect(await vaultView.isModuleCacheValid()).to.equal(false);
    });
  });
  ...
  describe('缓存清理与统计', function () {
    it('clearExpiredCache 应清除过期缓存并更新统计', async function () {
      ...
      await vaultView.connect(admin).pushUserPositionUpdate(userAddr, asset, 7, 3);
      const [totalBefore] = await vaultView.getCacheStats();
      expect(totalBefore).to.equal(1);

      await time.increase(301);
      await expect(vaultView.connect(admin).clearExpiredCache(userAddr))
        .to.emit(vaultView, 'CacheCleared')
        .withArgs(userAddr, anyValue);

      const [totalAfter] = await vaultView.getCacheStats();
    ...
    it('clearExpiredCache 不应清除未过期缓存', async function () {
      ...
      await vaultView.connect(admin).clearExpiredCache(userAddr);
      const [totalAfter] = await vaultView.getCacheStats();

      expect(totalAfter).to.equal(totalBefore);
      const [, , , moduleTsAfter] = await vaultView.getCacheStats();
      expect(moduleTsAfter).to.be.gt(0);
      const [, , isValid] = await vaultView.getUserPositionWithValidity(userAddr, asset);
      expect(isValid).to.equal(true);
    });
    ...
    it('getCacheStats 应返回正确的缓存统计', async function () {
      ...
      await le.borrow(userAddr, asset, 4, 0, 0);
      // 使用 pushUserPositionUpdate 创建缓存，这样会更新 _totalCachedUsers
      await vaultView.connect(admin).pushUserPositionUpdate(userAddr, asset, 9, 4);

      const [totalUsers, validCaches, cacheDuration, moduleCacheTimestamp] = await vaultView.getCacheStats();
  ...
  describe('业务推送与覆盖', function () {
    it('pushUserPositionUpdate 应覆盖缓存并标记有效', async function () {
      ...
      await vaultView.connect(admin).pushUserPositionUpdate(userAddr, asset, 21, 11);
